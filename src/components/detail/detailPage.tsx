...
const detailPage = (props: any) => {
  ...
  const indexData = [detailData[controlIndex]];
  let indexNum: string;
  let typeProve: any;
  ...
  return (
    <>
      {indexData[0] !== undefined &&
        indexData?.map((value: any, index: number) => (
          <DetailWide key={value.id}>
            <DetailTitle>
            ...
            {checkType === 'img' && (
              <TypeImg index={index} img={value.img} text={value.text} />
            )}
            {checkType === 'selector' && <TypeSpaceInfo index={index} />}
            {checkType === 'selectorvideo' && <TypeSpacevideo index={index} />}
            {checkType === 'Vr' && <TypeVr index={index} img={value.img} />}
            <Commonfooter>
